...
function renderContentField(
    ...
    if ($roles) {
        ...
        foreach ($roles as $role) {
            if (($role[0] != '!' && trim($role) != $_SESSION['mgrRole']) || ($role[0] == '!' && ltrim($role,
                        '!') == $_SESSION['mgrRole'])) {
                $hide = true;
            }
        }
    ...
    if (isset($data['tv'])) {
        ...
        $tvInherited = (substr($tvPBV, 0,
                8) == '@INHERIT') ? '<span class="comment inherited">(' . $_lang['tmplvars_inherited'] . ')</span>' : '';
        ...
        if ($data['tv']['type'] == 'richtext' || $data['tv']['type'] == 'htmlarea') {
            ...
            $richtexteditorIds[$editor][] = "tv" . $data['tv']['id'];
            $richtexteditorOptions[$editor]["tv" . $data['tv']['id']] = $tvOptions;
        }
        if ($data['tv']['type'] == 'image' && $showTvImage) {
            $renderTV = renderTypeImage($tvPBV, $data['tv']['id'], $modx->config['thumbWidth']);
        }
        $field .= renderFormElement($data['tv']['type'], $data['tv']['id'], $data['tv']['default_text'],
                $data['tv']['elements'], $tvPBV, '', $data['tv']) . $renderTV;
    ...
    if (isset($data['field'])) {
        ...
        if (isset($data['field']['type'])) {
            $default = isset($data['field']['default']) ? $data['field']['default'] : '';
            $elements = !empty($data['field']['elements']) ? $data['field']['elements'] : '';
            if (isset($content[$name]) && $content[$name] != '') {
                $value = $content[$name];
            } else {
                $value = $default;
            }
            $_ = renderFormElement($data['field']['type'], $name, $default, $elements, $value, '', $data['field']);
            $field .= str_replace(array('id="tv', 'name="tv'), array('id="', 'name="'), $_);
        } else {
            switch ($name) {
                case 'weblink':
			...
			<option value="0">(blank)</option>';
                    $rs = $modx->db->select("t.templatename, t.id, c.category",
                ...
                case 'parent':
                    ...
                    if ($parentlookup !== false && is_numeric($parentlookup)) {
                        $rs = $modx->db->select('pagetitle', $modx->getFullTableName('site_content'),
                            "id='{$parentlookup}'");
...
} elseif ($render_template_default) {
...
if ($modx->Event->name == 'OnDocFormTemplateRender') {
    ...
    $richtexteditorIds = !empty($richtexteditorIds) ? $richtexteditorIds : array();
    $richtexteditorOptions = !empty($richtexteditorOptions) ? $richtexteditorOptions : array();
    // Variables
            ...
            while ($row = $modx->db->getRow($rs)) {
                foreach ($mutate_content_fields as $k => $v) {
                    if (isset($v['fields']['tv' . $row['id']])) {
                        ...
                        $mutate_content_fields[$k]['fields'] = array_slice($mutate_content_fields[$k]['fields'], 0,
                                $position,
                                true) + array($row['name'] => $v['fields']['tv' . $row['id']]) + array_slice($mutate_content_fields[$k]['fields'],
                                $position, count($mutate_content_fields[$k]['fields']), true);
        ...
        if ($tab['title']) {
            ...
            foreach ($tab['fields'] as $fieldName => $field) {
                ...
                if (!empty($field['split'])) {
                    $tabContent .= '<label>' . $field['split']['title'] . '</label>';
                    $counter++;
                    $split_counter++;
                    if ($field['split']['hide']) {
                        $hide_counter++;
